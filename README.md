...
- History list shows **individual entries** with start time and duration (e.g. "Start: 10:00 · Duration: 1h 05m 30s"), color dot, color picker, delete. End times are not shown: durations exclude pauses, so "start + duration" is not when an activity actually ended.
...
- **Live activity indicator**: when the overlay is recording, a "● REC" entry appears at the top of today's history list showing the current activity name, start time, and tracked duration so far in green ("Start: 15:30 · Duration: 12m 05s so far"), amber with "❚❚ PAUSED" while paused.
